...
export default function DecorationUpdateForm(props) {
  const {
    id: idProp,
    decoration: decorationModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = decorationRecord
      ? { ...initialValues, ...decorationRecord }
      : initialValues;
    setName(cleanValues.name);
    setDescription(cleanValues.description);
    setSeason(cleanValues.season);
    setHoliday(cleanValues.holiday);
    setIsRentable(cleanValues.isRentable);
    setIsAvailable(cleanValues.isAvailable);
    setPrice(cleanValues.price);
    setImglink(cleanValues.imglink);
    setErrors({});
  };
  const [decorationRecord, setDecorationRecord] =
    React.useState(decorationModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Decoration, idProp)
        : decorationModelProp;
      setDecorationRecord(record);
    };
    queryData();
  }, [idProp, decorationModelProp]);
  React.useEffect(resetStateValues, [decorationRecord]);
  const validations = {
    name: [],
    description: [],
    season: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Decoration.copyOf(decorationRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "DecorationUpdateForm")}
      {...rest}
    >
      <TextField
        label="Name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || decorationModelProp)}
