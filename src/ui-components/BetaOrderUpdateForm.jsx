...
export default function BetaOrderUpdateForm(props) {
  const {
    id: idProp,
    betaOrder: betaOrderModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = betaOrderRecord
      ? { ...initialValues, ...betaOrderRecord }
      : initialValues;
    setCartItems(cleanValues.cartItems ?? []);
    setCurrentCartItemsValue("");
    setTotalPrice(cleanValues.totalPrice);
    setChosenInstallation(cleanValues.chosenInstallation);
    setHomeStreet(cleanValues.homeStreet);
    setHomeCity(cleanValues.homeCity);
    setHomeState(cleanValues.homeState);
    setHomeZip(cleanValues.homeZip);
    setHomeownerFirstName(cleanValues.homeownerFirstName);
    setHomeownerLastName(cleanValues.homeownerLastName);
    setHomeownerPhone(cleanValues.homeownerPhone);
    setHomeownerEmail(cleanValues.homeownerEmail);
    setErrors({});
  };
  const [betaOrderRecord, setBetaOrderRecord] =
    React.useState(betaOrderModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(BetaOrder, idProp)
        : betaOrderModelProp;
      setBetaOrderRecord(record);
    };
    queryData();
  }, [idProp, betaOrderModelProp]);
  React.useEffect(resetStateValues, [betaOrderRecord]);
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            BetaOrder.copyOf(betaOrderRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "BetaOrderUpdateForm")}
      {...rest}
    >
      <ArrayField
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || betaOrderModelProp)}
